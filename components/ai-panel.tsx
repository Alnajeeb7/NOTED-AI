...
import { X, Bot, Send, Loader2, RotateCcw, ChevronRight, ChevronDown, Zap, AlertTriangle, Check } from 'lucide-react'
...
const RATE_LIMIT_COOLDOWN_MS = 60_000 // 60s before clearing a rate limit
...
export function AiPanel({ workspaceId, width = 320 }: AiPanelProps) {
  ...
  // Auto-clear rate limits after cooldown
  useEffect(() => {
  ...
  const sendMessage = async () => {
    ...
    // Auto-switch if current model is rate-limited
    ...
    if (isRateLimited(selectedModel)) {
      ...
      if (fallback) {
        ...
        toast(`Switched to ${fallback.name} (previous model rate-limited)`, { icon: '🔄' })
      } else {
        toast.error('All models are rate-limited. Please wait a moment.')
        return
      }
    }
    ...
    try {
      ...
      if (res.status === 429) {
        ...
        toast.error(
          fallback
            ? `${GROQ_MODELS.find((m) => m.id === modelToUse)?.name} is rate-limited. Switch to ${fallback.name}?`
            : 'All models rate-limited. Please wait ~60s.',
          { duration: 5000 }
        )
        return
      }
      ...
      <div className="px-3 py-2 border-b border-border">
        ...
        >
          ...
          <span className={cn(
            ...
            isRateLimited(selectedModel) ? 'bg-amber-500' : 'bg-emerald-500'
          )} />
          ...
          {isRateLimited(selectedModel) && (
            <span className="text-[10px] text-amber-500 shrink-0">
              {getRateLimitSecondsLeft(selectedModel)}s
            </span>
          )}
            ...
            {GROQ_MODELS.map((m) => {
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    if (!limited) {
                      setSelectedModel(m.id as GroqModelId)
                      setModelOpen(false)
                    }
                  }}
                  disabled={limited}
                ...
                >
                  ...
                  <span className={cn(
                    ...
                    limited ? 'bg-amber-500' : 'bg-emerald-500'
                  )} />

                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <span className="text-xs font-medium">{m.name}</span>
                      <span className={cn('text-[9px] font-semibold px-1 py-0.5 rounded uppercase tracking-wider', TAG_STYLES[m.tag])}>
                        {m.tag}
                      </span>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      {limited && (
                        <span className="text-[9px] text-amber-500 ml-auto flex items-center gap-0.5">
                          <AlertTriangle className="w-2.5 h-2.5" /> Rate limited · {secsLeft}s
                        </span>
                      )}
                    </div>
                  </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {aiMessages.length === 0 && (
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p className="text-xs opacity-70">Create pages, search content, or draft notes.</p>
            <div className="mt-4 space-y-2">
              {[
                ...
                <button
                  key={s}
                  onClick={() => setInput(s)}
                  className="flex items-center gap-2 w-full text-left text-xs px-3 py-2 rounded-lg border border-border hover:bg-accent transition-colors"
                >
      ...
      <div className="p-3 border-t border-border">
        <div className="flex items-end gap-2 bg-muted rounded-xl px-3 py-2">
          <textarea
            ...
            placeholder={isRateLimited(selectedModel) ? 'Model rate-limited — switch above…' : 'Ask Noted AI…'}
            rows={1}
        ...
        <p className="text-center text-[10px] text-muted-foreground mt-1.5 opacity-50">
          {currentModel.name} · Enter ↵ to send
        </p>
      </div>
    </aside>
  )
